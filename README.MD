<h1 align="center">
  <br>
  <a><img src="./com.unai-gonzalez.wallpaper-deck.sdPlugin/imgs/wallpaper-deck-logo.png" alt="Wallpaper Deck Logo" height="200px"></a>
</h1>

<h3 align="center">Switch Wallpaper Engine wallpapers directly from your Stream Deck.</h3>

<p align="center">
  <a href="#key-features">Key Features</a> •
  <a href="#actions">Actions</a> •
  <a href="#setup">Setup</a> •
  <a href="#requirements">Requirements</a> •
  <a href="#download">Download</a> •
  <a href="#support">Support</a> •
  <a href="#license">License</a>
</p>

<h1 align="center">
  <img src="./com.unai-gonzalez.wallpaper-deck.sdPlugin/imgs/previews/1.png" alt="KOReadeck Banner">
</h1>

[![Get it on Marketplace](./com.unai-gonzalez.wallpaper-deck.sdPlugin/imgs/previews/marketplace.png "Get KOReadeck on Marketplace")](https://marketplace.elgato.com/product/wallpaper-deck-103d0c18-cddf-44b6-a324-e0f0c3307351)

[![Marketplace download badge](https://img.shields.io/badge/dynamic/json?logo=data:image/svg%2bxml;base64,&query=download_count&suffix=%20Downloads&label=Marketplace&labelColor=151515&color=204cfe&url=https%3A%2F%2Fmp-gateway.elgato.com%2Fproducts%2F103d0c18-cddf-44b6-a324-e0f0c3307351 "Marketplace download badge")](https://marketplace.elgato.com/product/wallpaper-deck-103d0c18-cddf-44b6-a324-e0f0c3307351)

**Wallpaper Deck** is a Stream Deck plugin that lets you switch [Wallpaper Engine](https://www.wallpaperengine.io/) wallpapers, playlists, and random picks instantly with a single button press, no alt-tabbing, no digging through menus. Every key shows a live preview of what it activates, turning your Stream Deck into a visual wallpaper gallery, and a built in search with favorites keeps your go to wallpapers one click away. Target a specific monitor or clone the wallpaper across all of them, right from the same button.

---

## Key Features

- **Instant wallpaper switching**, press a key, change your wallpaper.
- **Wallpaper preview on the button**, the key image shows the actual preview thumbnail of the wallpaper assigned to it.
- **Animated previews**, GIF wallpapers animate directly on the Stream Deck key. Can be toggled off per action.
- **Random wallpaper**, pick a random wallpaper from your entire collection with a single press. The property inspector shows the last picked wallpaper's preview.
- **Playlist support**, load and activate a Wallpaper Engine playlist from a single key, with a combined preview of every wallpaper in it.
- **Search and favorites**, filter the wallpaper list by title and mark your favorites so they always show up first. Favorites are stored locally by the plugin and are independent from the ones you mark inside Wallpaper Engine, since those depend on Steam.
- **Multi-monitor support**, target the default monitor, a specific screen, or clone the wallpaper to every detected monitor, per action.
- **Auto-detects your installation**, finds Wallpaper Engine and your Steam Workshop folder automatically via the Windows registry. No manual path configuration needed in most cases.
- **Works on Windows 10 and 11.**

---

## Actions

### Change Wallpaper

> _One press. One wallpaper._

Assigns a Wallpaper Engine wallpaper to a Stream Deck key. Pressing the key instantly activates that wallpaper on your desktop. The key displays the wallpaper's preview thumbnail so you always know what each button does at a glance.

Supports animated GIF previews on the key, toggle the **Animate** setting to disable it if you prefer a static thumbnail.

The wallpaper dropdown includes a search field to filter by title, and a favorite toggle next to it. Favorites are shown first in the list and are stored locally by the plugin, independent from Steam.

---

### Load Playlist

> _One press. Full playlist._

Loads and activates a Wallpaper Engine playlist from a single key. The key shows a combined preview of the wallpapers in the playlist, and the playlist order is displayed as a read only reference in the property inspector.

Playlists themselves (which wallpapers they contain, their order, and the switch timing) are managed inside Wallpaper Engine, under Wallpaper, Playlists.

---

### Random Wallpaper

> _One press. Random wallpaper._

Picks a random wallpaper from your Steam Workshop collection and activates it immediately. Each time you press the key a different wallpaper is selected.

---

#### Monitor selection

All actions include a **Monitor** setting with three modes:

- **Default monitor**, activates the wallpaper using Wallpaper Engine's own default behavior.
- **Specific monitor**, opens the wallpaper only on the screen you pick from the detected monitor list.
- **Clone to all monitors**, opens the same wallpaper independently on every detected screen.

> **Note:** "Specific monitor" and "Clone to all monitors" both require Wallpaper Engine's display mode to be set to **Wallpaper per monitor** (in the Wallpaper Engine app: monitor icon, Choose Monitor, Wallpaper per monitor). If it is set to "Stretch single wallpaper" or "Clone single wallpaper" instead, the key will show an error and the wallpaper won't change. This setting has to be switched manually inside Wallpaper Engine, there is no command-line way for the plugin to do it for you.

---

## Requirements

- **Stream Deck** 7.1 or later
- **Windows** 10 or 11
- **Wallpaper Engine** installed via Steam

---

## Download

Get the latest release from the [Stream Deck Marketplace](https://marketplace.elgato.com) or the [GitHub Releases page](https://github.com/unaigonzalezz/wallpaper-deck/releases).

---

## Setup

1. **Install the plugin** from the Stream Deck Marketplace or from the Releases page.
2. **Drag an action** onto any key on your Stream Deck:
   - **Change Wallpaper**, select a specific wallpaper from the dropdown.
   - **Load Playlist**, select a Wallpaper Engine playlist from the dropdown.
   - **Random Wallpaper**, activates a random wallpaper from your collection on each press.
3. **Press the key** to activate the wallpaper.

> **Note:** Wallpaper Deck auto-detects Wallpaper Engine and your Steam Workshop folder on first use. If detection fails, the Advanced section in the property inspector lets you set the paths manually.

---

## Support

If you would like to support development:

<a href="https://ko-fi.com/unaigonzalez" target="_blank">
  <img src="https://user-images.githubusercontent.com/7586345/125668092-55af2a45-aa7d-4795-93ed-de0a9a2828c5.png" width="160">
</a>

If you can't donate, leaving a ⭐ on the repo goes a long way, it genuinely makes my day.

---

## Future improvements

Feel free to suggest new features in the Discussions tab.

---

## Contributing

Pull requests are welcome. For major changes, open an issue first to discuss what you would like to improve.

---

## Thanks

- [Wallpaper Engine](https://www.wallpaperengine.io/) for the wallpaper platform that made this possible.
- [Google](https://google.com) for Material Icons.

---

## License

[MIT](https://choosealicense.com/licenses/mit/)

---
